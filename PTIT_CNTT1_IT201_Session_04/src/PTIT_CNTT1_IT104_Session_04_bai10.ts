...
function studentHasCompleted(CourseManager:CourseManager[]){
       ...
       for(let course of course2.courses){
           ...
           for(let students of course.students){
               if(students.hasCompleted){
                   ...
                   console.log(
                       `${students.id.padEnd(10)} | ${students.name.padEnd(20)} | ${students.email.padEnd(25)}| ${String(students.hasCompleted).padEnd(10)}| ${students.ComponentPoint ? calculatorComponentPoint(students.ComponentPoint).toFixed(2).padEnd(10) : "NULL".padEnd(10)}`
                   );
               }
           }
       }
...
function calculateAverageScore(CourseManager:CourseManager[]){
...
const calculatorPointOfCourse = (student:Student[])=>{
    let total = 0;
    let numberStHasC = 0;
...
function printCourseReport(manager: CourseManager[]){
    let i = 1;
